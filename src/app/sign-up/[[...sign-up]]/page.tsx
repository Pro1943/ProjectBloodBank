    ...
    <div className="flex min-h-screen bg-[#F8FAFC]">
      <div className="hidden w-[420px] shrink-0 flex-col justify-between border-r border-[#E2E8F0] bg-white p-10 lg:flex">
        <div>
          ...
          <div className="mt-12">
            <div className="inline-block rounded-full border border-teal-200 bg-teal-50 px-3 py-1 text-xs font-semibold text-[#0D9488]">
              Create account
            </div>
            <h1 className="mt-4 text-3xl font-bold text-[#0F172A]">Join the network.</h1>
            <p className="mt-3 text-[#64748B]">
              Sign up as a hospital to post emergency requests, or as a donor to find where your blood type is needed most.
            </p>
          </div>
          <div className="mt-10 space-y-4">
            {[
              { icon: "🏥", text: "Hospitals post emergency needs instantly" },
              { icon: "🩸", text: "Donors matched by blood type and location" },
              { icon: "📍", text: "Coordinate donation camps in your area" },
            ].map((item) => (
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-10">
        <div className="w-full max-w-md">
          <div className="mb-6 lg:hidden">
            ...
            <h1 className="text-center text-2xl font-bold text-[#0F172A]">Create your account</h1>
          </div>
          <SignUp routing="path" path="/sign-up" signInUrl="/sign-in" forceRedirectUrl="/auth/redirect" />
        </div>
      </div>
    </div>
  );
